...
这张表帮你快速确认 API、公式、排错入口和报告字段，不承担概念教学。它是一份快速索引，
不适合从头学。如果某个条目只剩一行结论、你却解释不了它为什么成立，就回到第 01～09 章查正文。
...
下面的最小模板包含 import、`@jit` 工厂、`@T.prim_func` 和边界保护，可以当作实验的固定
起点。先让内核跑对，再谈优化：
...
这一节把最常用的 API 按用途分组。查之前先想清楚你要做哪类操作——声明并行结构、分配内存、
搬运数据，还是归约/同步——再对到相应小节；每张表第一列是 API 名，后两列分别是作用和
要注意的地方。这个「先想类别、再查名字」的顺序，比在完整列表里逐项翻找更快。
...
API 查得再多，还是要有一份能脱口而出的标准结构。下面这个 tiled GEMM 模板应当成为你写
任何矩阵乘内核时的肌肉记忆——注意累加器用 fp32，这是精度语义里最容易漏的一点：
...
写完内核之后的问题，是"怎么编译、怎么跑、怎么测"。下面这块代码把编译、调用、取源码、
计时和校验串成一条线，随后的表再补充几个关键参数的可选取值。这几行几乎是每个内核的
「固定收尾」，养成照走一遍的习惯，就少问自己一次「下一步该干嘛」：
...
调参空间较大、没法手测全部配置时，用 autotune。下面是最小模板。所有配置必须用同一批
输入，比较才有效；否则测出来的数字没有可比性：
...
def matmul(M, N, K, block_M=128, block_N=128, block_K=32,
    ...
    # 这里仅展示 autotune 包装；真实 kernel 内应把 A/B/C 声明为 [M,K]/[K,N]/[M,N]。
    @T.prim_func
    def kern(A, B, C):
...
精度选择直接影响吞吐与正确性，也是很多数值问题的来源。下表列出常见 dtype 的值、命名和
用途，方便你快速选型。多数情况下，选错 dtype 不只是慢，还可能让结果过不了正确性检查：
...
这类变量主要用来排查缓存或调试 lowering。下表列出变量名与作用，调试时按需查阅：
...
硬件数字随型号、形态、功耗配置和精度口径变化，速查表不保存一份容易过期的峰值副本。
...
这里顺带澄清一个常见混淆：HBM/GDDR 是显存的物理实现，不是 CUDA 代码里的独立通用作用域。
代码和数据流图用 global/device memory，性能报告再注明物理显存类型。

## 7. 常见报错与解决

报错往往比文档更能定位问题。下面的表按"报错/症状 → 原因与解法"组织，命中相似报错时
先看左列，再到右列找第一步该做的排查。一条报错信息里藏着的信息，常常比一整页 API 文档
更贴近你此刻的真实处境：
...
报错无法直接定位时，按问题类型选工具。下表给出每类目标优先使用的工具：
...
最后这一节是面试前的临场速记。下面是一段 30 秒自我介绍模板，把「算子 → 方法 → 数据 →
验证」组织成一段话。模板的作用是别漏掉必要的数字和条件：
...
到这里，速查表就用完了。请记住它的边界：查表解决的是"此刻怎么查"，不是"有没有学会"。
用完这张表就回第 14 章做收官项目——代码、测试、测量、归因和适用范围都齐了，这个内核才算
达到课程验收标准。速查表能帮你更快走到那里，但走不走得到，还得靠你自己把那些查来的词
一个个接成能跑的代码。
