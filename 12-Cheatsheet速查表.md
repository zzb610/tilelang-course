...
本章用于快速确认 API、公式、排错入口和报告字段，不承担概念教学。它是一份快速索引，不适合
从头学习。若某个条目只剩下一行结论而你无法解释其原因，应回到第 01～09 章查阅正文。
...
下面的最小模板包含 import、`@jit` 工厂、`@T.prim_func` 和边界保护，可以作为实验的固定
起点。先确认内核能够正确运行，再讨论优化：
...
是 API 名，后两列分别是它的作用和需要注意的地方。这个「先想类别、再查名字」的顺序，
比在完整列表中逐项查找更快。
...
计时和校验串成一条线，随后那张表补充了几个关键参数的可选取值。这几行几乎是每个内核的
「固定收尾」，养成照着走一遍的习惯，能少问自己「下一步该干嘛」：
...
当调参空间较大、难以手动测试全部配置时，可以使用 autotune。下面是最小模板。所有配置
必须使用同一批输入，这是保证比较有效的前提；否则测量结果不具备可比性：
...
精度选择直接影响吞吐与正确性，也是许多数值问题的来源。下表列出常见 dtype 的值、命名和
用途，便于快速确认应该使用哪一种类型。多数情况下，选错 dtype 不仅影响性能，还可能使结果
无法通过正确性检查：
...
这类变量主要用于排查缓存或调试 lowering。下表列出变量名与作用，调试时按需查阅：
...
先看左列，再到右列找第一步该做的排查动作。一个报错信息里藏着的信息，常常比一整页 API
文档更贴近你此刻的真实处境：
...
当报错无法直接定位时，应根据问题类型选择工具。下表给出每类目标优先使用的工具：
...
最后这一节服务于面试前的临场速记。下面是一段 30 秒自我介绍的模板，将「算子 → 方法 →
数据 → 验证」组织成一段话。模板的作用是避免遗漏必要的数字和条件：
...
用完这张表之后回到第 14 章完成收官项目——只有代码、测试、测量、归因和适用范围同时齐全，
这个内核才算达到课程验收标准。速查表能帮你更快地走到那里，但走不走的到，还是要靠你自己
把那些查来的词一个个接成能跑的代码。
