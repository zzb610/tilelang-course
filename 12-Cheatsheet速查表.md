...
本章用于快速确认 API、公式、排错入口和报告字段，不承担概念教学。它是你写到一半时伸手指向
的那本字典，不是从头读起的教材。若某个条目只剩下一行结论而你无法解释其原因，应回到第
01～09 章，而不是继续依赖速查表——查表能救急，救不了理解。
...
`@jit` 工厂、`@T.prim_func` 和一次边界保护，你可以把它当作所有实验的固定起点——先让东西
跑起来，再谈优化，这比对着空白编辑器纠结第一行更养手：
...
配置必须喂进同一批输入，这是让最终配置可信的前提——不然你就是在比较不同输入下的成绩，那
数字说明不了任何问题：
...
用途，帮你在一秒内确认"应该用哪个"——多数时候，选错了 dtype 不是性能差一点，而是结果
干脆不可信：
...
翻阅即可，无需死记——需要它们的时候多半是出了问题，而不是平时写代码：
...
当报错无法一眼定位时，按目标选对工具往往事半功倍。这张表给出每种目标的第一工具，省得
你在成堆的 profiler 和工具里犹豫先开哪一个：
...
数据 → 验证」压缩成一段话，填空即可——照背的好处不是显得流利，而是保证该报的数字一个
都不少：
