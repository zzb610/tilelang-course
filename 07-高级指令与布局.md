...
下面这张表回答「同一份数据在不同层面上的意义」，据此你能在出问题时定位到是哪一层出了岔：
...
这也正是老手打开一个慢内核时的排查顺序：先问数据摆在哪，再问 block 按什么顺序跑，最后才
怀疑是不是时序没重叠——三层从「静态摆放」到「动态执行」，一条条往下过，而不是一上来就
乱翻开关。
...
你可以跟着心算一遍：thread 0 读 word 0，thread 1 读 word 32，thread 2 读 word 64——每个
线程往右挪 32 个 word，也就是 `32 × 4B = 128B`，而 32 个 bank 一字排开正好覆盖 128B 的
一轮，于是每挪一步都回到原点，全部撞进 bank 0：
...
个物理地址，使特定的线程访问模式更少撞 bank。通常有三件事不会变：tensor 的逻辑 shape
...
这些 API 适合在基线、布局和流水线都已经验证后再使用。这一点我得先说清楚：它们大多是
给你兜底或微调的，不是「加上就快」的捷径；而且我在这里也不逐一展开它们的语法和参数——
这部分高度依赖目标架构与 TileLang 版本，展开反而容易过时。你只需要先记住它们各自服务谁、
动手前先确认哪件事，具体用法到用时再查当前版本文档即可：
