...
第 06 章里，FlashAttention 的 `T.gemm` 与 fragment 已经算对，但我们只把布局、同步和硬件
指令当作黑箱：答案正确，却不知道「为什么这样摆放会更快或更慢」。这一章就把这层黑箱打开。
当 GEMM 或 Attention 已经算对，但性能、布局或同步出现问题时，你要能先判断问题属于哪一层，
再选择合适的 TileLang 原语。我们不把 API 当作清单背诵，而是围绕一条主线展开：**逻辑 shape →
物理布局 → block 调度 → 数据时序 → 线程通信**。
...
阅读时建议先读 7.1～7.5，建立「一个 tile 如何被布局、计算和调度」的完整模型；再读 7.6～7.9，
理解异步拷贝、同步、warp 原语和原子操作各自解决什么问题；最后读 7.10～7.12，把这些知识
整理成版本检查、实验和排障流程。
