...
缺的一个概念是 thread binding——它在 TileLang 里几乎总是由布局推断顺带解决，见 7.1.1。

先记住一句判断法：**如果逻辑坐标没有变，但元素在片上内存中的物理位置变了，这是布局；
如果 block 的执行顺序变了，这是调度；如果读取和计算重叠了，这是时序。**
分析性能较低的内核时，可以按相同顺序排查：先确认数据布局，再检查 block 调度顺序，最后
判断搬运与计算是否重叠。这样可以从静态布局逐步检查到动态执行，避免同时修改多个开关。
...
`wait` 和 `barrier` 长得很像，你在很多示例里都看到它们成对出现，但它们是两码事：一个回答
「数据到了没有」，另一个回答「人齐了没有、大家都看见这笔数据了吗」。这两个问题偏偏都卡在
「拷贝完成」和「线程看见数据」之间，所以最容易混为一谈。
...
这些 API 适合在基线、布局和流水线都已经验证后再使用。它们主要用于处理特定架构需求或进行
