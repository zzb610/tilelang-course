# 第 07 章 高级指令与布局：把 tile 映射到硬件执行

第 05、06 章中的 tile 已经有了清楚的数学意义，但硬件并不直接执行一个抽象的二维矩阵。
shared memory 把元素分进 bank，fragment 把元素分给线程，block 又按照某种次序抵达 SM。
同一个逻辑 shape 因为物理摆放和执行次序不同，可能产生完全不同的访存冲突与指令路径。

“布局”因此不是一个单独旋钮，而是几套坐标关系的总称。本章沿着**逻辑 shape → 物理布局
→ block 调度 → 数据时序 → 线程通信**逐层下降。每下降一层，我们都先说明这一层改变什么、
不改变什么，再引入相应原语。这样，swizzle、MMA、异步搬运、warp 通信和原子操作便不再是
散落的高级 API，而是解决不同层次冲突的工具。

这些工具中有不少依赖 CUDA 架构与 TileLang 版本。正文提供的是判断问题的坐标系；具体
指令是否生成、同步要求是否满足，仍需结合当前目标、生成代码和 profiler 验证。

> **本章导航** 高级难度，预计 4–6 小时；前置是第 04～06 章里关于分块 GEMM、软件流水线、
> 共享内存和 fragment 的知识。运行范围以 CUDA 为主，TMA、warpgroup、Tensor Memory 等内容
> 必须结合目标架构和 TileLang 版本。学完你会留下一张 bank conflict/swizzle 示意图、一份
> 开关对比实验记录，以及一条高级优化排障路径。参考：[TileLang Instruction Guide](https://tilelang.com/programming_guides/instructions.html)、
> [TileLang Software Pipeline Guide](https://tilelang.com/programming_guides/software_pipeline.html)、
> [Layout Visualization](https://tilelang.com/tools/layout_visualization.html)、
...
下面这张表说明同一份数据在不同层面的含义，可用于判断问题出现在哪个层面：
...
这四层与 TileLang 论文提出的「四种调度空间」一一对应：memory layout、thread binding、
tensorization（选哪条硬件指令路径）与 pipeline。论文的核心设计就是把这四类调度从数据流里
解耦，先让编译器推断，再允许用户逐层接管。你在本章学的每个原语，都对应其中一层。
还缺的概念是 thread binding——它在 TileLang 里几乎总是由布局推断顺带解决，见 7.1.1。

先记住一句判断法：**如果逻辑坐标没有变，但元素在片上内存中的物理位置变了，这是布局；
如果 block 的执行顺序变了，这是调度；如果读取和计算重叠了，这是时序。**
分析性能较低的内核时，按相同顺序排查：先确认数据布局，再看 block 调度顺序，最后判断搬运
与计算是否重叠。这样从静态布局一路查到动态执行，避免同时改动多个开关。
...
所以布局推断不是「编译器把矩阵重新排版」这么简单，而是在回答：
...
推断的具体机制（论文 4.2 节）：编译器维护一张**LayoutMap**（每个 buffer 当前采用的布局），
并给算子分优先级——`T.gemm` 这类 Tensor Core 算子的布局约束最严、优先级最高，
elementwise 最灵活。推断按优先级从高到低逐级进行，每一级把能确定的 buffer 都定下来，
直到没有进展再降级。两个观察支撑这个设计：多个算子共享 buffer 时布局相互耦合；
不同算子对布局的严格程度不同。

一个具体例子：GEMM 输出 `C`（fragment，每行被 2 个线程各持一半）之后紧跟一个 bias 加法，
bias buffer `D` 的布局就必须「对齐」`C`——每行的元素要让两个线程都能取到，于是编译器给
`D` 自动生成 **replicate（复制）** 布局。这就是第 06 章 FlashAttention 里
`acc_o[i, j] /= logsum[i]` 这类「fragment 除以行向量」能直接写、不必手工广播的原因。
...
thread 0 读 word 0，thread 1 读 word 32，thread 2 读 word 64。每个线程的地址相差 32 个 word，
即 `32 × 4 B = 128 B`，而 32 个 bank 正好覆盖 128 B 的
一轮，因此列方向每增加一步都会回到同一个 bank，最终全部访问 bank 0：
...
遇到共享内存访问变慢时，先按现象归类，再决定处理方式。下面这张表回答「看到什么现象、
先查什么、可能的解法是什么」三件事：
...
个物理地址，减少特定线程访问模式中的 bank conflict。通常有三件事不会变：tensor 的逻辑 shape
不变，例如仍是 `[BM, BK]`；GEMM 的数学含义不变；global 输入和输出的逻辑坐标不变。变化
的是 shared/fragment 内部的线程分配和物理地址映射。
...
这里的关键是先写出三者的 shape，参数本身是次要的。shape 不匹配时，优先检查
...
API 名称断言「它一定用了某条 Tensor Core 指令」。唯一可靠的依据是目标架构、生成代码和 profiler。

论文（4.3 节）归纳了 TileLang 把「高层算子」接到「硬件指令」的三条通道，按控制力递增：

| 通道 | 写法 | 控制力 | 代价 |
|---|---|---|---|
| Tile Library | 默认路径（NVIDIA cute / AMD ck），`T.gemm` 内部调用 | 低：库替你选指令，但内部布局不可外部注解 | CUTLASS 模板展开约占 nvcc 编译时间的 90%（论文 NVCC 12.8 trace 数据） |
| C++ 源码注入 | `T.import_source` + `T.call_extern` 包裹手写 C++ 模板（如 dp4a） | 中：熟悉 C 风格，指令级可控 | 仍要维护 C++ 代码 |
| 内联 PTX | `T.ptx(...)` 直接发射指令（如 `mma.m16n8k32...`） | 高：逐指令控制，适合 warp 级操作 | 与目标架构强耦合，需自行保证对齐/类型 |

同一个运算在不同指令路径上吞吐量级完全不同：论文以 RTX 3090 为例，int8 乘加用标量
`IMAD` ≈ 17.8 TOPS、向量化 `DP4A` ≈ 71.2 TOPS、Tensor Core `MMA` ≈ 284 TOPS。
这正是「选对指令路径」收益的来源，也是第 05 章 `T.gemm` 一行的价值所在。
...
`T.copy`、`T.async_copy` 和 `T.tma_copy` 都能把数据从 global 搬到 shared，但它们的时序
承诺完全不同。用错一个，轻则性能不对，重则数据还没到位就被消费。下面这张表把三者各自的
用途和默认语义摆在一起：
...
`wait` 和 `barrier` 长得很像，你在很多示例里都看到它们成对出现，但它们是两码事：`wait`
回答「数据到了没有」，`barrier` 回答「人齐了没有、大家都看见这笔数据了吗」。这两个问题
偏偏都卡在「拷贝完成」和「线程看见数据」之间，所以最容易混为一谈。

一个 wait 不能替代线程间 barrier；反过来，一个 barrier 也不能证明 TMA 数据已经到达。普通
`T.Pipelined + T.copy` 的路径可能由 lowering 自动插入必要同步，但手工异步路径必须检查生成代码。

### 7.7.1 常见同步原语

同步原语按作用域从小到大排列，各自解决不同规模的问题。下面这张表回答「每个原语覆盖多大
范围、用在什么场景、有什么约束」：
...
warp 原语能快速交换少量数据，但它的覆盖范围很窄。动手前先分清「这次通信发生在哪个范围、
要不要经过 shared/global」，下面这张表把四种通信方式按范围从小到大排列：
...
因此，原子通常用于保证正确性或合并小规模结果，不是默认的性能优化。能通过改变数据分区
让每个 block 写独立地址时，优先消除冲突，再考虑 atomic。

## 7.10 专家级接口及其适用对象

这些 API 适合在基线、布局和流水线都已经验证后再使用。它们主要用来满足特定架构需求、做
细粒度调整，并不保证直接提升性能。本节只说明各接口的适用对象和使用前提；具体语法和参数
高度依赖目标架构与 TileLang 版本，应查阅当前版本文档。
...
`tilelang.tools.plot_layout` 可以直接绘制显式的 `T.Layout` 或 `T.Fragment`。对推断布局，
当前版本还可以通过 pass config 输出文本或 SVG 等格式。它适合回答两类问题：

- 哪个线程持有逻辑 tile 中的哪些元素？
- 一个显式 layout 在输入视图和输出视图中怎样重排坐标？

图只能说明映射，不能直接证明没有 bank conflict 或访存已经合并。正确顺序是：先画线程—元素
关系，再结合元素大小和地址算 bank/transaction，最后用 profiler 验证。

```python
from tilelang.tools import plot_layout

# layout_or_fragment 必须是当前版本支持的 T.Layout 或 T.Fragment。
plot_layout(layout_or_fragment, name="gemm_fragment", save_directory="out/layout")
```

若使用自动布局推断，优先按照当前官方文档的 `PassConfigKey.TL_LAYOUT_VISUALIZATION_*`
配置启用输出，不要从旧文章复制已经改名的环境变量或 pass 名称。
...
有了 baseline 之后，先根据现象判断问题落在哪一层，再决定改哪个开关。下面这张表把常见现象
和优先检查项一一对应：
...
这一章把「把 tile 交给硬件高效执行」分解成层层递进的问题。**布局推断**解决「每个线程/warp
持有哪些逻辑元素」，fragment 的 shape 不等于线程清单；**bank conflict** 是不同地址映射到同一
shared-memory bank，先根据 stride 算 bank，再决定是否 swizzle。两条 swizzle 必须分清：
**共享内存 layout swizzle** 改变 block 内的物理地址/线程映射，而 `T.use_swizzle` 改变 block
之间的调度顺序，两者不是同一个开关。**`T.gemm`** 是 tile 级矩阵乘入口，先用 shape、scope、
transpose 和累加语义检查，再判断底层指令。搬运上，**`T.copy`** 是默认同步语义，
`T.async_copy` 和 `T.tma_copy` 暴露更细的时序控制，必须分别处理 wait、barrier 和版本/架构
约束；**warp 原语**解决一个 warp 内的少量通信，**atomic** 解决多个 block 对同一地址的并发
更新，但可能引入热点。把这些串起来，高级优化的证据链是：**baseline → 一次改一个开关 →
正确性 → profiler/生成代码 → 性能结论**。

## 7.13 动手任务

在进入第 08 章之前，请完成下面任务，把布局与指令的判断练成反射：
