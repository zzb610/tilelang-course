...
这里故意写“全局/设备内存”，不把 HBM 当作一层通用编程抽象。HBM、GDDR 是显存的
物理实现；TileLang/CUDA 代码通常面对的是 global/device memory 语义。L1、L2 是否命中
由缓存路径决定，数据访问也不是固定地按“寄存器 → shared → L2 → HBM”逐级串行经过。
...
- 常见存储层次包含全局/设备内存、缓存、共享内存和寄存器；HBM/GDDR 是显存物理实现。
