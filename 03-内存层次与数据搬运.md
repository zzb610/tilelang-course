...
第 02 章我们学会了读懂并写出一个内核：声明张量、配置 block、用循环表达计算。可一切
性能问题的起点其实都藏在一个更朴素的问题里——数据现在在哪里。同一个数组，放在全局
内存、共享内存还是寄存器里，速度可以差一个数量级；谁能看到它，决定了你要不要搬运、
能不能复用。这一章就来回答三个问题：数据现在在哪里、谁能看到它、为什么要搬运或复用
它。在此基础上理解 shared、fragment、`T.copy` 和 tiling，并写出一个刻意保守的 softmax
...
阅读的建议是：先用相对关系理解内存层次，不先背硬件常数；再从 GEMM 的一个输出 tile
推导数据复用；然后学习 `T.copy` 和手写搬运的边界；最后把这些概念放进 softmax，用
profiler 验证「访存受限」只是一个假设，而不是结论。
