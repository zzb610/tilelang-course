...
第 02 章告诉我们如何表达一个内核，却还没有解释一个令人困惑的现象：两段做着相同数学
运算的程序，速度为什么可能相差许多倍。答案往往不在运算符里，而在数据抵达运算单元之前
走过的路上。

因此，本章暂时把“算什么”放到一边，专门研究“数据在哪里”。位置决定可见范围，可见范围
决定哪些线程能够共同使用一份数据，而复用次数又决定一次搬运是否值得。全局内存、共享内存
和 fragment 不是一串需要背诵的名词，它们共同构成“位置—可见性—复用”这一因果链。
...
这里有两个措辞上的讲究值得说清。其一，这里故意写“全局/设备内存”，不把 HBM 当作一层
通用编程抽象——HBM、GDDR 是显存的物理实现，而 TileLang/CUDA 代码通常面对的是
global/device memory 语义。其二，L1、L2 是否命中由缓存路径决定，数据访问也不是固定地
按“寄存器 → shared → L2 → HBM”逐级串行经过。
...
搞清楚「数据在哪」之后，自然的问题是：为什么要费劲把数据搬来搬去？答案藏在数据复用里。
以 GEMM `C[M,N] += A[M,K] @ B[K,N]` 为例，朴素写法每个输出元素都要读一整行 A 和一列
B，全局访存量 O(M·N·K)。但 A 的一个元素其实被 N 个输出复用、B 的一个元素被 M 个输出
复用。**把计算分块**（如每 block 算 128×128 的 C tile，K 维按 32 切）后，每个块只需
把 128×32 + 32×128 的数据搬进共享内存，就能反复使用——全局访存一下子降低一个数量级。
...
下面假设 `M/N/K` 都能被 `BM/BN/BK` 整除；不能整除时，把除法换成 `ceildiv`，并额外处理
边界 guard 或零填充。
...
其中最关键的一个理解是：**fragment 的「形状」是虚拟的**。你写 `(BM, BN)`，编译器通过
**布局推断（layout inference）**决定每个线程实际持有哪些元素。比如 MMA 片段里一个线程
持有 16×8 块中的 4 个元素。这个机制让你写「tile 级」代码，而不用管线程内如何摆放——
...
[官方 issue #2543](https://github.com/tile-ai/tilelang/issues/2543)（v0.1.11 报告，已随
#2636 修复）曾记录过一个值得内化的教训：whole-tile `T.copy` 越界时**静默忽略**
`T.annotate_safe_value` 注解、一律填 0，而逐元素 `T.Parallel` 加载路径却遵守注解——
同一份 60×60 数据拷进 64×64 tile、注解哨兵值 `-99`，`T.copy` 路径得到 0，
`T.Parallel` 路径得到 `-99`，且生成源码里根本搜不到 `-99` 字符串。
这给出两条通用测试方法，与具体版本无关：
...
前面的概念在 GEMM 里都比较抽象，现在用一个更简单的算子把它们都串起来。目标：对矩阵
`A[M, N]` 的每一行做 softmax。流程：每 block 处理一行，把行读进共享内存，先求行 max，
再求 exp 和 sum，最后归一化写回。体会 global → shared → register → global 的完整数据流。
