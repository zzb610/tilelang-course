...
从第 13 章的专项扩展走出来，现在是回到一个真实算子、把前面十三章全部收束成一次完整
交付的时候。前十三章里，你已经反复练习同一条闭环——写最小正确基线、测正确性、测性能、
只改一个变量、记录证据、固化配置——而这一章要你把这条闭环用在别人真正会调用的算子
上。Capstone 在全书的位置，是一个「从会写到交付」的总验收：它不再考你知不知道某个
API，而是考你能不能把一个需求澄清成契约、把契约变成测试、把测试变成两轮有证据的优化、
再把最优配置封装成知道何时启用、何时回退的模块。学完这一章，你留下的不再是一段跑得
通的代码，而是一个能解释自己每一个决定、又能被他人复现的项目。
...
阅读的顺序建议这样走：先把选题缩到一个清楚的输入分布和设备，再建立不可跳过的参考实现；
随后完成「正确但不一定快」的 TileLang 版本。只有测试矩阵全部通过，才进入 profile 和优化。
最后把最优 kernel 放进 dispatch 表，并保留库 fallback。

## 14.1 选题：先选工作负载，不先选高级指令

Capstone 的成败，一半在动笔之前就已经决定了。很多人一上来就挑最新的指令和最高级的
...
完成下面任务，再宣布这一次 Capstone 交付完成。它们不是新增的负担，而是把本章每一节的
成果整理成可以交给他人复现的产物：
...
把每个答案背后的取舍讲出来，这次 Capstone 才算真正内化：
