...
收官项目要完成的转变，是从“会写一个内核”走向“能交付一个算子”。算子契约决定测试，
测试守住优化边界，profile 为改动提供理由，dispatch 与 fallback 则把局部最优放回完整系统。
如果参考库在目标形状上更快，项目也不因此失败；只要差距已经被解释，适用范围已经写清，
停止优化同样是一项有证据的工程决定。

因此，本章不再引入新的 TileLang 原语。它把全书出现过的思想重新组织成一次真实交付：先
选择有明确输入分布的工作负载，写下契约与参考实现，再进行两轮单变量优化，最后交付测试、
性能矩阵、dispatch 规则和可复现报告。没有目标设备时可以完成设计与 compile-only 检查，
但不能据此通过性能验收。
...
选定算子之后、编写 kernel 之前，先用一页纸明确算子计算什么，以及在哪些范围内保证正确。
shape、dtype、布局和数值语义中的遗漏，都可能在后续测试或集成阶段导致错误。因此，下面的
表格应在实现前填写完整。
...
契约写完之后，应先准备两项内容：一个可信的参考实现，以及
一套能把边界逐一类分出来的测试矩阵。参考实现的职责是表达数学语义，它不必快，也不必
采用与目标 kernel 相同的调度；正因为两者不同，它才能成为独立的对照。
...
当参考实现和测试矩阵就位，你才第一次允许自己写 TileLang 内核。但这一版的内核目的
不是快，而是正确。基线阶段的任务是确保数据流、边界处理和测试方法可靠，为后续优化提供
可以复现的对照版本。
...
基线正确并且差距已经定位，优化才获得入场资格。这里有一条贯穿全章的纪律：每一轮只改变
一个主要因素，从而让每一点性能变化都能归因。下面这张表先给出常见的观测信号，再对应到
可能的下一步，以及验证那一步需要的新证据：
...
访问模式不受益。不要为了让报告「全是正收益」而删除失败实验——恰恰是那几次没奏效的尝试，
由此可以判断该算子适合哪些优化；只报告成功实验的数字无法得到这一结论。

## 14.6 公平比较库实现

优化走完两轮之后，你需要一个外部参照来回答「值不值得」。这个参照通常是库，但库对比
只有在两边条件等价时结果才可信，否则较好的数字可能只来自测量条件差异。因此这一节先说明公平比较的
约束，再讲它的局限，最后补上两道筛查来补足局限。
...
「对库比较」有一个隐藏的循环论证：PyTorch 既当基线、又当「好」的定义——你拿它来当尺子，
却忘了这尺子本身可能只是「最常用的那个」，而不是「最快的那个」。针对这个盲区，"Correct
but Slow" 研究（[arxiv 2607.04454](https://arxiv.org/abs/2607.04454)）提出用
**两道互补筛查**一起判断内核是否真的高效：
...
有了最优 kernel，交付还没有结束：真实系统不会因为某一个 shape 变快，就把所有请求交给
同一个 kernel。这一节处理的是「如何让这块代码安全地进入生产」——先决定谁能走它的路径，
再决定谁能安全回退，最后补上与框架集成时必须检查的细节。
...
至此，把前面所有环节收束成一张可以据此打分、也据此交付的验收表。它回答的问题是：「这
个算子现在能不能交给别人用？」下面这张表分七个类别，每一类的通过条件都对应前面某几节
的成果；它既是检查清单，也是你复盘时的提纲。
