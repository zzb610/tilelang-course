...
收官项目的成败，一半在动笔之前就已经决定了。我见过太多人一上来就挑最新的指令和最高级的
feature，仿佛选难的题才能证明能力；但做了几轮优化之后，我又一再回到同一个结论：先有真实
需求，再谈该用什么手段。这不是保守，而是前十三章反复现身的一条顺序——先问负载长什么样，
手段自然会跟出来。所以这一节先把「什么算合格的选题」讲清楚。
...
可以从官方 examples 的算子族中选择。下面这张表按「方向—起点—难点—坑」四列展开，
帮你在动手前避开最典型的第一目标错误：
...
选定算子之后、动手写 kernel 之前，先把需求钉死在一页纸上。所谓契约，就是把「这个算子
到底要算什么、在什么范围内保证正确」逐项写下来；这一步每省一行，后面就会在测试或集成
阶段以 bug 的形式还回来。因此这一节给出的不是建议，而是动手前必须填完的一张表。
...
契约写完之后，正确的下一步不是写 kernel，而是先立起两样东西：一个可信的参考实现，和
...
不是快，而是正确。基线阶段的唯一任务，是让数据流、边界和可测试性都站得住，为后续
优化留下一个可回溯的锚点。
...
帮你划出了「这算子吃哪套优化」的边界，这是只报成功数字换不来的判断。

## 14.6 公平比较库实现

优化走完两轮之后，你需要一个外部参照来回答「值不值得」。这个参照通常是库，但库对比
只有在两边条件真正等价时才可信，否则漂亮的数字只是测量偏差。因此这一节先讲公平比较的
...
研究报告了两个用法要点，每一条都来自一张真实摆不平的矛盾里得到的教训：
...
下面这些问题用来检验你是否能把这一章学到的交付能力讲清楚——当你能在不看笔记的情况下
把每个答案背后的取舍讲出来，这次收官项目才算真正内化：
