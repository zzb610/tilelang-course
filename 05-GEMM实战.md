...
到目前为止，我们分别讨论了程序结构、数据位置和时间重叠。GEMM 是这些概念第一次真正
汇合的地方。它的数学定义只有一行，但一个输出元素需要沿 K 维不断累加，一块输入又会被
许多输出重复使用；这使数据复用、片上容量、流水线和矩阵指令同时进入同一个设计。

本章不把 GEMM 当作一份“高性能模板”交给读者，而是保留它的演化过程。最初版本直接从
全局内存取数，数学上正确，却浪费了输入复用；第二个版本引入 shared tile；随后流水线改变
搬运与计算的时序，布局和 block 调度再处理更细的硬件约束。最后，自动调优只在已经理解的
参数空间中寻找组合，而不替代这些推理。
