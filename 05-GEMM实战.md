...
本章从一个数学正确但数据复用不足的版本出发，逐版加入共享内存分块、软件流水线、边界
...
理论下限约为 0.4 毫秒。这个数值可以作为性能上限的参照，用于判断实测结果与硬件上限的差距。
...
全局内存累加（K 维每步都读全局）。这个版本用于建立对照，后续版本可以据此分析数据复用带来的变化。
...
调参过程应保持单变量原则：修改一个开关 → 重新编译 → 测量 → 记录。v3 将开关声明为函数
参数，以便重复实验。这里还需要区分两个名称相同、作用不同的 swizzle：
...
如果已经测试上述开关但 TFLOPS 仍然较低，应先检查调优空间和 L2 驻留问题。"Correct but
...
这两项都属于结构性问题，仅修改算子代码无法解决，需要调整调度空间和缓存策略。第 09 章给出
