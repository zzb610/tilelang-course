...
框架中的矩阵乘只需要一次函数调用，而 GPU 内核必须显式处理数据复用、片上容量、流水线和
矩阵指令。一个输出元素要沿 K 维不断累加，一块输入又会被多个输出重复使用。本章通过五个
版本的 GEMM，逐步说明这些因素如何共同决定实现。

我们从一个数学正确但数据复用不足的版本出发，逐版加入共享内存分块、软件流水线、边界
...
先明确问题和度量单位。计算 `C[M,N] = A[M,K] × B[K,N]`（行主序）。计算量是 `2·M·N·K`
FLOPs。以 `M=N=K=4096` 为例，计算量约为 137 GFLOPs；在 fp16 峰值为 330 TFLOPS 的 GPU 上，
理论下限约为 0.4 毫秒。拿这个数当性能上限的参照，就可以看出实测结果离硬件上限有多远。
...
### 5.1.1 先明确精度语义：fp32 的 `T.gemm` 可能使用 TF32

在写任何优化之前，先处理一个会让「正确性」结论失效的陷阱——必须把它放在最前面，因为一旦
...
先写一个**正确但没有数据复用优化**的版本。每个 block 计算一个 `BM×BN` 输出 tile，直接从
全局内存累加（K 维每步都读全局）。写它是为了做对照：后面每个版本的提速，都能在它身上找
到原因。为突出访存差异，下面的 v0 只用于整除尺寸；尾部处理留到 5.4：
...
一句完成 tile 级矩阵乘。具体会落到哪类 Tensor Core/后端指令，由目标 GPU、dtype 和
...
真实的矩阵尺寸几乎从不是 4096 这种漂亮的数——`M=1003`、`N=517`、`K=70` 才是常态。
M、N、K 不整除时，要分别处理三件事：
...
调参过程应保持单变量原则：修改一个开关 → 重新编译 → 测量 → 记录。v3 把开关声明为函数
参数，以便重复实验。这里还要区分两个名称相同、作用不同的 swizzle：
...
如果已经测了上述开关，TFLOPS 仍然较低，应先检查调优空间和 L2 驻留问题。"Correct but
Slow" 研究在 A100/GH200 上归纳了大尺寸 GEMM 的两个常见原因：
...
1. **默认调优网格缺配置（RC2a）**。它测的 16384² GEMM 用教程默认的
   ...
   结论：**autotune 的空间由用户定义，缺少的维度不会自动出现**——第 08 章的 config
...
2. **L2 驻留（RC2b）**。16384² 的 A+B+C 工作集约 1.6 GB，远超 A100 的 40 MB L2。
   ...
   缺少这类启发式时只有 49.4% 命中、呈 DRAM-bound。由此可见，大尺寸下
   `T.use_swizzle`（光栅化调度）改善 L2 局部性的意义远超小尺寸，值得单独开关实验。

这两项都属于结构性问题，光改算子代码解决不了，需要调整调度空间和缓存策略。第 09 章给出
...
评估缺口。它的样本和设备有限，不能推广成「所有 TileLang 内核都慢」；能引出的正确结论是，
真实交付至少要回答两组问题：
